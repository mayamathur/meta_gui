...
setwd(here())
source("tests_human_inspection/helper_testthat.R")
...
#### test1 gbc_prepped.csv file - for JL to fix
```{r  error=TRUE}

# @@MM: I fixed the confusing error from confounded_meta (see improved output below).
#  But now the website is throwing a different error about "arguments imply differing number of rows: 1, 0"
# Please fix that so it throws confounded_meta's error
# Then please update the plot as well

## jl something weird going on here -- seems like sometimes it gives the MM error below and sometimes gives the "arguments imply differing number of rows: 1, 0" when just re-running the same code:

d = read.csv("tests_human_inspection/Datasets for website test/gbc_prepped.csv", stringsAsFactors = FALSE)

# note: log(0.9) = -0.04575749056
...
# @@MM: JL, fix this please:
# "non-numeric argument to mathematical function"
# this happens with the plot when passing on log scale
# @@JL: fixed, there was a typo in the variable call
...
# @@JL, fix this please:
# "non-numeric input"
# does not match confounded_meta
# @@JL: fixed, there was a typo in the variable call
...
#### test10 - for JL to fix
```{r error=TRUE}
# @@MM: This one previously had Tmin<1. This wasn't wrong because it represents bias in the 
#  other direction, but for clarity I changed the package output to by definition set Tmin=Gmin=1
#  in these cases. Now these are not being displayed on the app at all, maybe becuase their CIs are [NA, NA]. 
# @@JL: we currently have in the app that if p < r it outputs the  "Not applicable. This is already the case, even with no bias, given your pooled effect size, threshold, and choice of tail." message...is this incorrect?
confounded_meta(method="parametric",
                q=log(.5),
                r=0.75,
                tail="below",
...
#@@MM: This also has the "non-numeric argument to mathematical function" error
#@@JL: fixed
sens_plot(method = "parametric",
          type="line",
          q=0.1823216,
